...
void setup() {
  ...
  server.on("/",rootHandler);
  server.on("/index",IndexFileHandler); // index.html on spiff
  server.serveStatic("/index2",SPIFFS, "/index2.html");
...
void loop() {
  ...
  static uint32_t pMillis2=0;
  if(millis()-pMillis2>1000){ // milli seconds
      pMillis2 = millis();
